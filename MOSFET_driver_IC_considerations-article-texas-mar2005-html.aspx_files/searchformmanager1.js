...
function SearchDetails(sPath, SearchType) {
    if (document.forms[0].txtSearch.value != "") {
        if (SearchType != '') {
            if (sPath != "")
                window.location.href = sPath + "SearchArticle.aspx?Qry=" + document.forms[0].txtSearch.value + "&aType=" + SearchType;
            else
                window.location.href = "SearchArticle.aspx?Qry=" + document.forms[0].txtSearch.value + "&aType=" + SearchType;
        }
        else {
            if (sPath != "")
                window.location.href = sPath + "SearchArticle.aspx?Qry=" + document.forms[0].txtSearch.value;
            else
                window.location.href = "SearchArticle.aspx?Qry=" + document.forms[0].txtSearch.value;
        }
    }
}

function WIInstructWindow() {
    var myRef = window.open('Instruction.htm', 'mywin', 'left=20,top=20,width=500,height=300,toolbar=0,resizable=0');
}

function EnterFunc(keyCode, sPath) {
    var SearchType = getRadioValue(document.getElementsByName('SearchType'));
